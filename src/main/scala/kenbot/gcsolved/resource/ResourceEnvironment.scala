...
import kenbot.gcsolved.resource.io.ResourceIO
import com.sun.org.apache.bcel.internal.util.ClassPath.Dir
import kenbot.gcsolved.resource.pack.ResourcePackager
import kenbot.gcsolved.resource.pack.DefaultResourcePackager
import kenbot.gcsolved.resource.io.DefaultResourceIO
...
object ResourceEnvironment {
  ...
  def apply(homeDirectoryFile: File, resourceIO: ResourceIO = DefaultResourceIO, resourcePackager: ResourcePackager = DefaultResourcePackager): ResourceEnvironment = new ResourceEnvironment {
    ...
    private def getLibraryRefFromPackedFile(packedFileName: String): String = {
      val extLength = packedFileName.length - packedFileName.findLastIndexOf('.' ==)
